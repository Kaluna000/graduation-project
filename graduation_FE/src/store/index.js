...
import {getInstanceLife} from "@/api/getInstanceLife";
...
const actions = {
    ...
    podApplication({commit},applicationInfo){
        const {instanceName,username,podName,minCPU,maxCPU,
            minMemory,maxMemory,minEphemeralStorage,maxEphemeralStorage,price,duration} = applicationInfo
        return new Promise((resolve, reject) => {
            podApplication(instanceName,username,podName,minCPU,maxCPU,
                minMemory,maxMemory,minEphemeralStorage,maxEphemeralStorage,price,duration).then(response => {
                commit('') //提交一个 mutation，通知状态改变
                resolve(response) //将结果封装进 Promise
            }).catch(error => {
                reject(error)
    ...
    getInstanceLife({commit},username){
        return new Promise((resolve, reject) => {
            getInstanceLife(username).then(response => {
                commit('ChangeInstanceLifeList',response) //提交一个 mutation，通知状态改变
                resolve(response) //将结果封装进 Promise
            }).catch(error => {
                reject(error)
            })
    ...
    renew({commit},renewInfo){
        const {username,podName,price,duration} = renewInfo
        return new Promise((resolve, reject) => {
            renew(username,podName,price,duration).then(response => {
                commit('') //提交一个 mutation，通知状态改变
                resolve(response) //将结果封装进 Promise
            }).catch(error => {
                reject(error)
...
const mutations = {
    ...
    ChangeInstanceInfoList(state){
        ...
        for (let i=0;i<state.namespacedPodList.length;i++){
            state.instanceInfoList.unshift({
                ...
                // life:state.instanceLifeList[i].life
                nodePort:state.namespacedServiceList.filter((ins) => {return ins.name === state.namespacedPodList[i].name})[0].nodePort,
                life:state.instanceLifeList.filter((ins) => {return ins.podname === state.namespacedPodList[i].name})[0].life
            })
        }
    },
    ...
    ChangeInstanceLifeList(state,response){
        state.instanceLifeList = response.data
    },
    ChangeRenewPodName(state,value){
        state.renewPodName = value
    }
}
